...
NcodeImageResizer.prototype.createWarning = function() {
	...
	mimg.src = NcodeImageResizer.TEMPLATE_PATH+'/images/ncode_icon.png';
	mimg.width = 16;
	mimg.height = 16;
	mimg.alt = '';
	mimg.border = 0;
...
NcodeImageResizer.prototype.scale = function() {
	...
	if(this.img.width < 450) {
		this.warningTextNode.data = NcodeText['ncode_imageresizer_warning_small'];
	} else if(this.img.fileSize && this.img.fileSize > 0) {
		this.warningTextNode.data = NcodeText['ncode_imageresizer_warning_filesize'].replace('%1$s', this.originalWidth).replace('%2$s', this.originalHeight).replace('%3$s', Math.round(this.img.fileSize/1024));
		//mtext.data = '<phrase 1="'+this.originalWidth+'" 2="'+this.originalHeight+'" 3="'+Math.round(this.img.fileSize/1024)+'">$NcodeText[ncode_imageresizer_warning_filesize]</phrase>';
	} else {
		this.warningTextNode.data = NcodeText['ncode_imageresizer_warning_no_filesize'].replace('%1$s', this.originalWidth).replace('%2$s', this.originalHeight);
		//mtext.data = '<phrase 1="'+this.originalWidth+'" 2="'+this.originalHeight+'">$NcodeText[ncode_imageresizer_warning_no_filesize]</phrase>';
	}
	...
	switch(NcodeImageResizer.MODE) {
		...
		default:
			...
			if(this.warning != null) {
				this.warningTextNode.data = NcodeText['ncode_imageresizer_warning_fullsize'];
				this.warning.width = this.img.width;
